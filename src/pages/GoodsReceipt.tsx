  ...
  const fetchData = async () => {
    try {
      ...
      const [orderData, historyData, rabData] = await Promise.all([
        ...
        api.get('goods_receipts', 'select=*,po:purchase_orders(po_number,project_id,rab_project_id),material:materials(name,unit),variant:material_variants(merk),worker:worker_masters(name)&order=tanggal.desc&limit=20'),
        api.get('rab_projects', 'select=id,project_id,unit_id,keterangan')
      ]);

      const enrichedOrders = (orderData || []).map((o: any) => {
        const matchingRab = (rabData || []).find(r => r.id === o.rab_project_id);
        return { ...o, rab_title: matchingRab?.keterangan || null };
      });

      const enrichedHistory = (historyData || []).map((h: any) => {
        const po = h.po;
        const matchingRab = po ? (rabData || []).find(r => r.id === po.rab_project_id) : null;
