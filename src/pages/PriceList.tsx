...
import { FileText, Upload, Printer, ArrowLeft, Trash2, Download } from 'lucide-react';
        ...
        const defaultDocs: MarketingDocument[] = [
          {
            id: '1',
            type: 'pricelist',
            name: 'Price List Griya Asri - Maret 2026.pdf',
            file_url: '#',
            created_at: new Date().toISOString()
          }
        ];
        const allDocs = getMockData<MarketingDocument>('marketing_documents', defaultDocs);
        setDocs(allDocs.filter(d => d.type === 'pricelist'));
        return;
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (isMockMode) {
        ...
        setDocs(updatedDocs.filter(d => d.type === 'pricelist'));
        return;
      }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500">Daftar harga unit properti terbaru</p>
          </div>
        </div>
      ...
      >
        <div className="space-y-4">
          <div 
            ...
            onClick={async () => {
              const name = prompt('Masukkan nama file:', `Price List ${new Date().toLocaleDateString('id-ID')}.pdf`);
              if (!name) return;
